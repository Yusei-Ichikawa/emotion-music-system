    ...
    while _running:
        try:
            ...
            if cmd[0] == "quit":
                ...
                print_help()
            # elif cmd[0] == "bpm" and len(cmd) >= 2:
            #     if cmd[1].startswith(("+","-")):
            #         try:
            #             delta = int(cmd[1])
            #             with state_lock:
            #                 target = max(40, min(220, state["active_bpm"] + delta))
            #                 state["pending_bpm"] = target
            #         except:
            #             pass
            #     else:
            #         try:
            #             v = int(cmd[1])
            #             with state_lock:
            #                 state["pending_bpm"] = max(40, min(220, v))
            #         except:
            #             pass
            #     with state_lock:
            #         print(f"BPM (next bar) -> {state['pending_bpm']}")
            # elif cmd[0] == "swing" and len(cmd) >= 2:
            #     try:
            #         v = float(cmd[1])
            #         with state_lock:
            #             state["swing"] = max(0.0, min(0.5, v))
            #         print(f"Swing -> {state['swing']}")
            #     except:
            #         pass
            # elif cmd[0] == "prog" and len(cmd) >= 2:
            #     name = cmd[1].lower()
            #     if name in PROGRESSIONS:
            #         with state_lock:
            #             state["pending_prog"] = name
            #         print(f"Progression (next bar) -> {name}")
            #     else:
            #         print(f"Unknown progression: {name}. Available: {list(PROGRESSIONS.keys())}")
            # elif cmd[0] == "key" and len(cmd) >= 2:
            #     try:
            #         val = int(cmd[1])
            #         with state_lock:
            #             state["key_shift"] = max(-24, min(24, state["key_shift"] + val))
            #         print(f"Key shift -> {state['key_shift']} semitones")
            #     except:
            #         pass
            # elif cmd[0] == "vol" and len(cmd) >= 3:
            #     part = cmd[1].lower()
            #     try:
            #         v = max(0, min(127, int(cmd[2])))
            #     except:
            #         continue
            #     with state_lock:
            #         if part == "melody":
            #             state["mel_vol"] = v; fs.cc(CH_MELODY, 7, v)
            #         elif part == "chord":
            #             state["chd_vol"] = v; fs.cc(CH_CHORD, 7, v)
            #         elif part == "guitar":
            #             state["gtr_vol"] = v; fs.cc(CH_GUIT, 7, v)
            #         elif part == "bass":
            #             state["bass_vol"] = v; fs.cc(CH_BASS, 7, v)
            #         else:
            #             print("part must be: melody | chord | guitar | bass")
            #             continue
            #     print(f"Volume {part} -> {v}")
...
# ========= 再生ループ =========
print(f"♪ 再生中: BPM={state['bpm']}, Swing={state['swing']}.  ターミナルで 'help' を入力できます。")
